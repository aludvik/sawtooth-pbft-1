...
impl PbftNode {
    ...
    pub fn on_peer_message(
        ...
        let multicast_hint = if msg_type.is_multicast() {
            ...
            handlers::multicast_hint(state, &pbft_message)
        } else {
        ...
        match msg_type {
            ...
            PbftMessageType::Checkpoint => {
                ...
                if !state.is_primary() && state.mode != PbftMode::Checkpointing {
    ...
    pub fn retry_backlog(&mut self, state: &mut PbftState) -> Result<(), PbftError> {
        ...
        if let Some(msg) = self.msg_log.pop_backlog() {
            debug!("{}: Popping from backlog {}", state, msg.message_type);
    ...
    fn _broadcast_message(
        ...
        self.on_peer_message(&peer_msg, state)
    }
...
mod tests {
    ...
    fn multicast_protocol() {
        ...
        assert!(node.on_peer_message(&garbage_msg, &mut state0).is_err());
